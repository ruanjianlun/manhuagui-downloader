...
class MangaDownloaderSession:
    def __init__(self, address, path, session=None):
    ...
    def getAbstraction(self, url):
        req = self.session.get(url).content
    ...
    def downloadPg(self, url, e, m, localPath):
        ...
        for i in range(10):
            try:
                res = self.session.get(url, params={'e': e, 'm': m}, headers=header, timeout=10)
